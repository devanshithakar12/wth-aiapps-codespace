...
ask_sarah_controller = func.Blueprint()


@ask_sarah_controller.function_name("ask_sarah_assistant")
@ask_sarah_controller.route(route="assistants-ask-sarah", methods=["POST"],
                            auth_level=AuthLevel.ANONYMOUS)
def ask_sarah(req: func.HttpRequest) -> func.HttpResponse:
    ...
    assistant_config: AssistantConfig = settings.get_assistant_config(assistant_name=AssistantName.SARAH)

    system_message = assistant_config["system_message"]
    tools_configuration = assistant_config["tools"]

    util = ToolUtils(AssistantName.SARAH, system_message, tools_configuration, conversation_id)
    ...
    final_response = (api_response.add_response_header("x-assistant-name", AssistantName.SARAH)
